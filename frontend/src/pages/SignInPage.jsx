...
import { Mail, Lock, Eye, EyeOff, Loader2, ArrowRight, CheckCircle, Smartphone, Clapperboard } from 'lucide-react';
...
const SignInPage = () => {
    ...
    const handleGoogleLogin = () => {
        ...
        setTimeout(() => {
            toast.dismiss();
            toast.success('Google sign-in (UI only)');
        }, 1000);
    };
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-8 bg-white overflow-y-auto">
                <div className="w-full max-w-sm space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-xs text-gray-400 font-bold uppercase tracking-wider">or sign in with email</span>
                        <div className="h-px bg-gray-100 flex-1"></div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                ...
                                <Link to="/auth/forgot-password" className="text-sm font-bold text-[var(--color-primary)] hover:underline">Forgot?</Link>
                            </div>
                            <div className="relative group">
                        ...
                        <div className="flex items-center">
                            <label className="flex items-center cursor-pointer gap-3 group">
                                <div className="relative flex items-center">
                                    <input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="peer w-5 h-5 rounded border-gray-300 text-[var(--color-primary)] focus:ring-[var(--color-primary)]" />
                                </div>
                                <span className="text-sm font-medium text-gray-600 group-hover:text-gray-900">Remember me for 30 days</span>
                            </label>
                        </div>
                    ...
                    <p className="text-center text-gray-500 font-medium">
                        Don't have an account? <Link to="/auth/sign-up" className="text-[var(--color-primary)] font-bold hover:underline">Sign up for free</Link>
                    </p>
                </div>
            </div>
        </div>
